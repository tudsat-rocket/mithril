...
async fn main(low_priority_spawner: Spawner) {
    ...
    #[cfg(not(feature = "gcs"))]
    let (flash, flash_handle, settings) = Flash::init(SpiDevice::new(spi3, spi3_cs_flash), usb_flash)
        .await
        .map_err(|_e| ())
        .unwrap();
    ...
    let vehicle = Vehicle::init(
        imu, acc, mag, baro, gps_handle, power, usb, radio, flash_handle, can_handle, leds, buzzer, recovery, settings,
    );
