...
impl<SPI: SpiDevice<u8>> LSM6<SPI> {
    ...
    async fn read_sensor_data(&mut self) -> Result<(), SPI::Error> {
        ...
        self.gyro = Some(self.gyro_scale.scale_raw_values(Vector3::new(gyro_y.saturating_neg(), gyro_z, gyro_x.saturating_neg())));
        self.accel = Some(self.accel_scale.scale_raw_values(Vector3::new(accel_y.saturating_neg(), accel_z, accel_x.saturating_neg())));
