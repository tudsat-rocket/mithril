...
impl FlashHandle {
    ...
    pub fn write_settings(&mut self, settings: Settings) -> Result<(), ()> {
        self.request_sender
            .try_send(FlashRequest::WriteSettings(settings))
            .map_err(|_e| ())
    }

    pub fn write_message(&mut self, msg: DownlinkMessage) -> Result<(), ()> {
        self.request_sender
            .try_send(FlashRequest::WriteMessage(msg))
            .map_err(|_e| ())
    }

    pub fn read(&mut self, address: u32, size: u32) -> Result<(), ()> {
        self.request_sender
            .try_send(FlashRequest::Read(address, size))
            .map_err(|_e| ())
    }
    ...
    pub async fn init(spi: SPI, usb: FlashUsbHandle) -> Result<(Self, FlashHandle, Settings), FlashError<SPI::Error>> {
        ...
        let settings = loop {
            match flash.read_settings().await {
                ...
                Err(e) => {
                    error!(
                        "Failed to read settings from flash ({:?}), reverting to defaults.",
                        Debug2Format(&e)
                    );
        ...
        let result = {
            ...
            for i in 0..(PAGE_SIZE / CHUNK_SIZE) {
                ...
                for attempt in 0..3 {
                    result = self
                        .driver
                        .write((self.pointer as usize) + (i * CHUNK_SIZE), &chunk)
                        .await;
                    if result.is_err() {
                        continue;
                    }
                    ...
                    let read_back = self
                        .driver
                        .read(self.pointer + (i * CHUNK_SIZE) as u32, CHUNK_SIZE as u32)
                        .await;
    ...
    async fn write_settings(&mut self, settings: &Settings) -> Result<(), FlashError<SPI::Error>> {
        ...
        // separate scope so we get rid of these buffers before reading back
        {
            ...
            for i in 0..(FLASH_SETTINGS_SIZE as usize / PAGE_SIZE) {
                self.driver
                    .write(PAGE_SIZE * i, &sector[(PAGE_SIZE * i)..(PAGE_SIZE * (i + 1))])
                    .await?;
