...
#[allow(dead_code)]
const WARNING_MELODY: [Note; 4] = [Note::note(C, 5, 500), Note::pause(200), Note::note(C, 5, 500), Note::pause(9000)];

#[allow(dead_code)]
const SHORT_WARNING_MELODY: [Note; 2] = [Note::note(C, 5, 500), Note::pause(500)];
#[allow(dead_code)]
const NO_BATTERY_ATTACHED_MELODY: [Note; 4] =
    [Note::note(F, 5, 400), Note::pause(10), Note::note(D, 5, 100), Note::pause(10)];
...
impl<TIM: CaptureCompare16bitInstance> Buzzer<TIM> {
    ...
    pub fn apply_settings(
        ...
        self.drogue_warning_note = Note::frequency(
            drogue_output_settings.output_warning_frequency, drogue_output_settings.output_warning_time,
        );
        self.main_warning_note = Note::frequency(
            main_output_settings.output_warning_frequency, main_output_settings.output_warning_time,
        );
    }
    ...
    pub fn tick(&mut self, time: u32, battery_status: Option<BatteryStatus>) {
        ...
        if let Some(freq) = self.current_frequency() {
            self.block
                .moder()
                .modify(|w| w.set_moder(self.pin, vals::Moder::ALTERNATE));
            ...
            self.block
                .moder()
                .modify(|w| w.set_moder(self.pin, vals::Moder::OUTPUT));
